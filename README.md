...
| `internal/cli` | the user-facing CLI shared by both front-ends: `start` (one-command onboarding: setup → register → summary), `setup` (config, seeds, systemd system service, OS resolver wiring, interactive sudo), `uninstall` (one-command removal: every freens* service + unit files + OS wiring; `-trust`/`-purge` extras), `register <alias>` (positional or `-alias`; cooldown-safe retries), `name`, `status` (plain-language; `-v` raw), `doctor --fix`, `backup`/`-restore` (key bundle + RESTORE.txt), `upgrade` (self-update from GitHub releases: verify + install in place, config migrations, service restart; `-check` compares only), `revoke <name>` / `forget <name>` (tombstone / tombstone + key deletion), plus the full operator verb set (`gen-key`/`mine-claim`/`make-record`/`publish`/`resolve`/`get`/`transfer`/`rotate`/`recover`/`verify-recovery`/`demo`; every seed flag accepts hex or `@keyfile`); admin-socket transport with standalone `-peers` fallback; first-timer quickstart + typo suggestions | §6.4, §7, §8, §9.4 |
...
see "NAT traversal" in `contrib/README.md`; `-upnp` (default on) asks the
LAN's router to forward the DHT port and advertises the external address
with zero configuration, `-stun <server>`
...
`-advertise` > `-upnp` > `-turn-relay` > `-stun`, with graceful fallback to
