...
// webui_reserved_test.go — the §7.7 registration gate on the web UI register
...
func TestOpsRegisterRefusesReservedAlias(t *testing.T) {
	...
	for _, alias := range []string{"com", "localhost", "org"} {
		...
		if err == nil {
			t.Fatalf("register %s via web UI: want the §7.7 reserved-alias error, got %+v", alias, res)
		}
